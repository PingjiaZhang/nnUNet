...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        levelset = np.zeros(self.seg_shape, dtype=np.float32)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            levelset_path = os.path.join(os.path.split(self._data[i]['data_file'])[0], 'gt_levelset',
                                         i + '_levelset.npz')
            try:
                levelset_data = np.load(levelset_path, self.memmap_mode)['levelset'][np.newaxis]
            ...
            levelset_data = np.copy(levelset_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                    valid_bbox_y_lb:valid_bbox_y_ub,
                                    valid_bbox_z_lb:valid_bbox_z_ub])
            ...
            levelset[j] = np.pad(levelset_data, ((0, 0),
                                                 (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                 (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                 (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
        ...
        return {'data': data, 'seg': levelset, 'properties': case_properties, 'keys': selected_keys, 'levelset': levelset}
